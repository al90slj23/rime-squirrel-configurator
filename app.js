...
`;
  ...
  // 生成专属部署命令
  document.getElementById('btnGenerateCmd').addEventListener('click', ()=>{
    const schemaName = schemaId.value || 'luna_pinyin';
    ...
    // 3. 生成 emoji 词库（如果需要）
    const emojiDict = enableEmoji.checked ? generateEmojiDict() : '';

    // 4. 生成 rime.lua（如果需要）
    const rimeLua = (enableLunar.checked || enableEmoji.checked) ? generateRimeLua() : '';

    // 5. 使用 base64 编码所有文件内容（支持中文）
...
DEPLOYEOF
    ...
    // 8. 提示用户
    const btn = document.getElementById('btnGenerateCmd');
    const originalText = btn.textContent;
    btn.textContent = '✓ 命令已生成';
